(function ($){
    ...
    berocket_apply_styles_for_button = function ($parent) {
        ...
        $parent.find('.infinite_loader_btn_load .infinite_button').replaceWith($button);
        $parent.find('.infinite_loader_btn_load .infinite_button').trigger('infinite_loader_button_changed');
        ...
        $settings.each(function () {
            ...
            if ($style != 'custom_css') {
                if ($field == 'border') {
                    ...
                    $button.css($style, value + 'px solid ' + $(this).parents('.form-table').first().find('.btn_border_color').val());
                } else {
                    if ($style == 'text') {
    ...
    $(document).ready( function () {
        setTimeout(function() {
            $('.infinite_loader_btn_load .infinite_button').each(function() {
                berocket_apply_styles_for_button($(this).parents('.form-table').first());
            });
        }, 10);
        ...
        $(document).on('change', '.form-table .infinite_loader_btn_settings, .bg_btn_color, .txt_btn_color, .btn_border_color', function () {
            berocket_apply_styles_for_button($(this).parents('.form-table').first());
        });
        ...
        $(document).on('click', '.infinite-loader-set-load-more-options', function ( event ) {
            ...
            $( '.form-table .infinite_loader_btn_settings, .form-table .lmp_button_settings_hover' ).each( function ( i, o ) {
